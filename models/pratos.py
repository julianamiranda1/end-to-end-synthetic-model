...
class PratoInput(BaseModel):
    ...
    preco_promocional: float = Field(
        default=None, gt=0, description="Preço promocional em reais"
    )
    ...
    def preco_promocional_menor_que_preco(cls, v, values):
        ...
        if desconto > 0 and desconto / values["preco"] > 0.5:
            raise ValueError(
                "O desconto promocional não pode ser maior que 50% do preço original"
            )
        return v
...
class DisponibilidadeInput(BaseModel):
    disponivel: bool = Field(
        description="Indica se o prato está disponível para pedido"
    )
